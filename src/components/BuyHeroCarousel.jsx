...
export const BuyHeroCarousel = ({ properties = [], onOpenEnquiry, onSelectResidence }) => {
  ...
  const currentProp = slides[currentIndex];
    ...
    >
      ...
      <div className="relative z-20 pt-28 sm:pt-32 px-6 sm:px-12 max-w-[1440px] mx-auto w-full">
        <nav className="flex items-center gap-2 text-xs text-gray-300">
          ...
          <span className="text-[#D4AF37] font-medium tracking-wide">Buy Properties</span>
        </nav>
      </div>
      ...
      <div className="relative z-20 px-6 sm:px-12 max-w-[1440px] mx-auto w-full py-8 sm:py-12 my-auto">
        <div className="max-w-3xl">
          ...
          {/* Title with Smooth Typography (Clutter labels removed per user request) */}
          <h1 
          ...
          >
            ...
            <span className="font-light">{currentProp.location}</span>
          </div>
          ...
          >
            {currentProp.overview}
          </p>

          {/* Key Feature Chips */}
          <div 
            key={`feats-${currentIndex}`}
            className="flex flex-wrap gap-2 sm:gap-3 mb-8 animate-in fade-in slide-in-from-bottom-2 duration-700"
          >
            {currentProp.features && currentProp.features.slice(0, 3).map((feat, fIdx) => (
              <div 
                key={fIdx} 
          ...
          <div className="flex flex-wrap items-center gap-4 sm:gap-6 pt-2">
            <div>
              <span className="block text-[11px] uppercase tracking-widest text-[#D4AF37] font-semibold">Starting Price</span>
              <span className="text-2xl sm:text-3xl font-bold text-white tracking-tight">{currentProp.price}</span>
            </div>

            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => onOpenEnquiry && onOpenEnquiry(currentProp)}
              ...
              <button
                type="button"
                onClick={() => onSelectResidence && onSelectResidence(currentProp)}
            ...
            {slides.map((slide, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{slide.title.split(',')[0].replace('AR Homes ', '')}</span>
                </button>
              );
            })}
          </div>
...
export default BuyHeroCarousel;
